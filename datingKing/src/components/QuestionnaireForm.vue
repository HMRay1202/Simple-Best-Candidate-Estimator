...
const savedCandidates = ref<CandidateData[]>([])

const submitForm = async () => {
  if (!candidateForm.value) return
  
  await candidateForm.value.validate((valid: boolean) => {
    if (valid) {
      ...
      savedCandidates.value.push(candidateData)
      saveToLocalStorage()
...
const resetForm = () => {
  if (!candidateForm.value) return
  candidateForm.value.resetFields()
}
...
const deleteCandidate = (index: number) => {
  ElMessageBox.confirm(
    ...
    savedCandidates.value.splice(index, 1)
    saveToLocalStorage()
    ElMessage({
      type: 'success',
      message: '删除成功',
...
const exportData = () => {
  if (savedCandidates.value.length === 0) {
    ElMessage({
      message: '没有可导出的数据！',
      type: 'warning'
    })
  ...
  const dataStr = JSON.stringify(savedCandidates.value, null, 2)
...
const saveToLocalStorage = () => {
  localStorage.setItem('datingCandidates', JSON.stringify(savedCandidates.value))
}

const loadFromLocalStorage = () => {
  const savedData = localStorage.getItem('datingCandidates')
  if (savedData) {
    savedCandidates.value = JSON.parse(savedData)
  }
}
